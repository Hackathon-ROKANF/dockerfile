...
export class MoneyParser {
   ...
   * 예: "2억 5천만원" -> 250000000
   */
  static toWon(priceText: string): number | null {
    ...
    try {
      ...
      if (choMatch) {
        total += parseFloat(choMatch[1]) * 1_000_000_000_000
      }
      ...
      if (cheonMatch) {
        total += parseFloat(cheonMatch[1]) * 1_000
      }
      ...
      if (total === 0) {
        ...
        if (numberMatch) {
          ...
          total = number >= 1000 ? number : number * 10_000
        }
      }
   ...
   * 예: 250000000 -> "2억 5천만원"
   */
  static toKorean(won: number): string {
    if (!won || won <= 0) return '0원'

    const eok = Math.floor(won / 100_000_000)
    const man = Math.floor((won % 100_000_000) / 10_000)
    const won_remainder = won % 10_000
