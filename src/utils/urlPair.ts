...
export interface UrlPairResult {
  saleUrl: string
  rentUrl: string
 ...
 */
export function generateUrlPair(currentUrl: string, baseUrl: string = 'https://www.bdsplanet.com'): UrlPairResult | null {
  try {
    // currentUrl이 절대/상대든 관계없이 URL 객체로 정규화
    const u = new URL(currentUrl, baseUrl)
    ...
    const saleUrl = new URL(salePath, `${u.origin}`).toString()
    const rentUrl = new URL(rentPath, `${u.origin}`).toString()
 ...
 * @returns 인코딩된 주소 문자열
 */
function extractEncodedAddress(pathname: string): string {
 ...
 */
export function tryDecodeKorean(encoded: string): string | undefined {
  ...
  try {
    ...
    if (euc && !looksMojibake(euc)) return euc
  } catch (_) {
    // EUC-KR 디코딩 실패
  }
 ...
 * 유효한 한국 주소인지 검사
 */
function isValidKoreanAddress(str: string): boolean {
 ...
 */
export function generateOppositeTabUrl(currentUrl: string, baseUrl: string = 'https://www.bdsplanet.com'): string | null {
