...
export class BdsPlaywrightCrawler {
   ...
   */
  private generateUrlPair(currentUrl: string, address: string): UrlPair | null {
    try {
   ...
   */
  private async extractPriceWithTabSwitch(page: Page, tabType: string, jsonResponses: string[]): Promise<number | null> {
    try {
   ...
   * 더 넓은 범위의 셀렉터로 가격 추출
   */
  private async extractPriceWithBroadSelectors(page: Page): Promise<number | null> {
